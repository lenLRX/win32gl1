...
void affineTransform(float* x, float* y, float originX, float originY, float theta,float r);//仿射变换
...

{
	...
	if (Director::getTheInstance()->getTotalScene())
	{
		...
		for (vector<Sprite*>::iterator it = thescene->getSpriteList().begin(); it != thescene->getSpriteList().end(); it++)
		{
			...
			bl.x = origin.x - size.width / 2;
			bl.y = origin.y - size.height / 2;

			br.x = origin.x + size.width / 2;
			br.y = origin.y - size.height / 2;

			tr.x = origin.x + size.width / 2;
			tr.y = origin.y + size.height / 2;

			tl.x = origin.x - size.width / 2;
			tl.y = origin.y + size.height / 2;
			float r = sqrt(size.width*size.width + size.height*size.height);
			affineTransform(&bl.x, &bl.y, origin.x, origin.y, (rotf + 90) / 180 * M_PI,r);
			affineTransform(&br.x, &br.y, origin.x, origin.y, (rotf + 90) / 180 * M_PI,r);
			affineTransform(&tr.x, &tr.y, origin.x, origin.y, (rotf + 90) / 180 * M_PI,r);
			affineTransform(&tl.x, &tl.y, origin.x, origin.y, (rotf + 90) / 180 * M_PI,r);
			glPrint("%f", bl.x);
			glLoadIdentity();
			...
			glTexCoord2f(0.0f, 0.0f); glVertex3f((bl.x) / WINDOW_WIDTH, (bl.y) / WINDOW_HEIGHT, 0.0f);    //左下
			glTexCoord2f(0.0f, 1.0f); glVertex3f((br.x) / WINDOW_WIDTH, (br.y) / WINDOW_HEIGHT, 0.0f);      //右下
			glTexCoord2f(1.0f, 1.0f); glVertex3f((tr.x) / WINDOW_WIDTH, (tr.y) / WINDOW_HEIGHT, 0.0f);      //右上
			glTexCoord2f(1.0f, 0.0f); glVertex3f((tl.x) / WINDOW_WIDTH, (tl.y) / WINDOW_HEIGHT, 0.0f);      //左上
...
void affineTransform(float* x, float* y, float originX, float originY, float theta ,float r)//仿射变换
{
	...
	returnX = (*x)*cosTheta - (*x)*sinTheta -originX - originX*cosTheta + originY * sinTheta;
	returnY = (*x)*sinTheta + (*y)*cosTheta -originY - originX*sinTheta - originY * cosTheta;
	float _temp = sqrt((returnX - originX)*(returnX - originX) + (returnY - originY)*(returnY - originY));
	returnX *= r / _temp;
	returnY *= r / _temp;
	(*x) = returnX;
	(*y) = returnY;
